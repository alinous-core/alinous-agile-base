...
public class NodeTreeGetPrevNode extends AbstractSystemFunction
{
	public static String QUALIFIED_NAME = "NODETREE.GETPREVNODE";
	public static String TREE_NAME = "treeName";
	public static String NODE_ID = "nodeId";

	public NodeTreeGetPrevNode()
	{
			...
			throws ExecutionException, RedirectRequestException
	{
		...
		if (stmtStack.size() != this.argmentsDeclare.getSize()) {
			throw new ExecutionException(
					"Number of the function's arguments is wrong : "
							+ QUALIFIED_NAME);// i18n
		}
		...
		IScriptVariable treeNameVariable = newValRepo.getVariable(ipath,
				context);
		...
		if (!(treeNameVariable instanceof ScriptDomVariable)
			...
			throw new ExecutionException(
					"Type of the function's argument is wrong : "
							+ QUALIFIED_NAME);// i18n
		}
		...
		NodeConfig config = context.getCore().getConfig().getNodeTreeConfig()
				.getNode(treeName);

		// do not have to close connection
		DataSrcConnection con = null;
		try {
			con = context.getUnit().getConnectionManager()
					.connect(config.getDatastore(), context);

			NodeModel node = NodeModelUtils.getPrevNodeModel(con, config, nodeId);
			...
			throw new ExecutionException(e, "There is no tree resource : "
					+ treeName); // i18n
		}finally{
			con.close();
		}
	...
	private ScriptDomVariable node2Dom(NodeModel model)
	{
		...
		addValue(modelDom, NodeModel.NUM_CHILDREN, Integer.toString(model
				.getNumChildren()));
		addValue(modelDom, NodeModel.POS_IN_LEVEL, Integer.toString(model
				.getPosInLevel()));
		addValue(modelDom, NodeModel.LEVEL, Integer.toString(model.getLevel()));
		addValue(modelDom, NodeModel.PARENT_ID, Integer.toString(model
				.getParentId()));
