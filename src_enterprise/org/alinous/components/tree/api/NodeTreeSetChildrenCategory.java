...
public class NodeTreeSetChildrenCategory extends AbstractSystemFunction
{
	public static final String QUALIFIED_NAME = "NODETREE.SETCHILDRENCATEGORY";
	
	public static final String TREE_NAME = "treeName";
	public static final String NODE_ID = "nodeId";
	public static final String CATEGORY = "category";
	
	public NodeTreeSetChildrenCategory()
	{
	...
	public IScriptVariable executeFunction(PostContext context, VariableRepository valRepo) throws ExecutionException, RedirectRequestException
	{
		...
		try {
			...
			handleChildren(con, config, category, nodeModel);
	...
	private void handleChildren(DataSrcConnection con, NodeConfig config, 
			String category, NodeModel nodeModel) throws ExecutionException, DataSourceException
	{
		List<NodeModel> list = NodeModelUtils.getChildren(con, config, nodeModel);
		Iterator<NodeModel> it = list.iterator();
		while(it.hasNext()){
			NodeModel model = it.next();
			handleSelf(con, config, category, model);
		}
	}

	private void handleSelf(DataSrcConnection con, NodeConfig config, 
			String category, NodeModel nodeModel) throws ExecutionException, DataSourceException
	{
		handleChildren(con, config, category, nodeModel);
