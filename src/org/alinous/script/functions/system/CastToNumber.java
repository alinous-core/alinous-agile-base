...
public class CastToNumber extends AbstractSystemFunction
{
	public static String QUALIFIED_NAME = "CAST.TONUMBER";
	...
	public IScriptVariable executeFunction(PostContext context, VariableRepository valRepo) throws ExecutionException, RedirectRequestException
	{
		...
		boolean successInt = true;
		try{
			Integer.parseInt(domValue.getValue());
		}catch(Throwable e){	
			successInt = false;
		}
		try{
		...
		if(successInt){
			domValue.setValueType(IScriptVariable.TYPE_NUMBER);
			retVal = domValue;
		}else{
			double tmpValue = Double.parseDouble(domValue.getValue());
			tmpValue = Math.floor(tmpValue);
			int intVal = (int)tmpValue;
			
			domValue.setValueType(IScriptVariable.TYPE_NUMBER);
			domValue.setValue(Integer.toString(intVal));
	...
	public String codeAssistString() {
		return "Cast.toNumber($arg0)";
	}

	@Override
	public String descriptionString() {
		return "Cast the variable into the Number type.\n" +
				"The Number type means integer.\n";
	}
