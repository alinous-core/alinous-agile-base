...
public class DropTableSentence implements ISQLSentence
{
	private int line;
	...
	public boolean execute(PostContext context, VariableRepository valRepo) throws ExecutionException
	{
		...
		try {
			con.dropTable(this.table.extract(context, valRepo, null, null, con.getTypeHelper()));
		} catch (DataSourceException e) {
			//con.close();
			throw new ExecutionException(e, "Failed in droping table at line " + this.getLine());
		}
	...
	{
		return this.table.isReady(context, valRepo, adjustWhere);
	}
