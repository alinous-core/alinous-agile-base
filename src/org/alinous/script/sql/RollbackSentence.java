...
public class RollbackSentence implements ISQLSentence
{
	private int line;
	...
	public boolean execute(PostContext context, VariableRepository valRepo) throws ExecutionException, RedirectRequestException
	{
		...
		try {
			...
			throw new ExecutionException(e, "execute rollback failed on getting connection");
		}
		
		// execute begin
		try {
			con.rollback(strtrxIdentifier);
		} catch (DataSourceException e) {
			con.close();
			throw new ExecutionException(e, "executing rollback failed.");
		} catch(Throwable e){
			throw new ExecutionException(e, "execute rollback failed");
		}finally{
			//context.getGlobalTrxIds().remove(strtrxIdentifier);
	...
	{
		Element rollbackElement = new Element(IExecutable.TAG_EXECUTABLE);
		
		rollbackElement.setAttribute(IExecutable.ATTR_CLASS, this.getClass().getName());
		
		parent.addContent(rollbackElement);
	}
