...
public class ModuloStatement implements IStatement
{
	...
	public IScriptVariable executeStatement(PostContext context, VariableRepository valRepo) throws ExecutionException, RedirectRequestException
	{
		...
		if(this.opeStatements.size() != 1){
			throw new ExecutionException("Cannot modulo."); // i18n
		}
	...
	private void checkValue(IScriptVariable val) throws ExecutionException
	{
		if(!(val instanceof ScriptDomVariable)){
			throw new ExecutionException("Cannot execute modulo operation to array."); // i18n
		}
		...
		if(!type.equals(IScriptVariable.TYPE_NUMBER) &&  !type.equals(IScriptVariable.TYPE_DOUBLE)){
			throw new ExecutionException("Cannot modulo String nor boolean."); // i18n
		}
	}
